...
import { getWalletLabel, getWalletColor, isWalletComingSoon } from '../utils/walletConfig';
    ...
    >
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {walletGroups.map((group) => {
          const comingSoon = isWalletComingSoon(group.value);
          const personalItems = group.items.filter((wallet) => !wallet.isShared);
          const sharedItems = group.items.filter((wallet) => wallet.isShared);
          const personalKey = `${group.value}-personal`;
          const sharedKey = `${group.value}-shared`;
          ...
          // Render Coming Soon card for disabled categories
          if (comingSoon) {
            return (
              <Card
                key={group.value}
                ...
                sx={{
                  ...
                  borderRadius: 2,
                  opacity: 0.6,
                  background: `linear-gradient(180deg, ${alpha(group.color, 0.04)}, transparent 45%)`,
                }}
              >
                <CardContent sx={{ py: 3 }}>
                  ...
                  <Typography variant="body2" color="text.disabled" sx={{ mt: 1, ml: 3.5 }}>
                    This category will be available in a future update.
                  </Typography>
                </CardContent>
              </Card>
            );
